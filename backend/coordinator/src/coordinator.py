...
class RelayCoordinator:
    ...
    def __init__(self, redis_client: Redis):
        ...
        self.distribution_interval = int(
            os.getenv("RELAY_DISTRIBUTION_INTERVAL_SECONDS", 30)
        )
        self.last_distribution = 0
        self.collector_timeout = int(
            os.getenv("COLLECTOR_TIMEOUT_SECONDS", 60)
        )  # 1 minute
    ...
    async def get_active_collectors(self) -> List[str]:
        ...
        for collector_id in collectors:
            ...
            collector_id_str = (
                collector_id.decode("utf-8")
                if isinstance(collector_id, bytes)
                else collector_id
            )
            ...
            if has_registration and current_time - registration_time < grace_period:
                logger.info(
                    f"Including recently registered collector {collector_id_str} in active collectors (no heartbeat yet)"
                )
                active_collectors.append(collector_id)
                continue
            ...
            logger.debug(
                f"Collector {collector_id_str} has no heartbeat, including in active collectors during grace period"
            )
            active_collectors.append(collector_id)
    ...
    async def should_redistribute_relays(self) -> bool:
        ...
        distribution_requested = self.redis.get(
            "dvmdash:settings:distribution_requested"
        )
        ...
        if outdated:
            logger.info(
                f"Found {len(outdated)} outdated collectors that need relay updates"
            )
            return True
    ...
    async def get_outdated_collectors(self) -> List[str]:
        ...
        try:
            ...
            for collector_id in collectors:
                ...
                collector_data = self.redis.hgetall(
                ...
                for key in collector_data:
                    ...
                    if key_str == "config_version":
                        ...
                        value = collector_data[key]
                        config_version_value = int(
                            value.decode("utf-8") if isinstance(value, bytes) else value
                        )
                        break

                if not has_config_version or config_version_value != int(
                    current_version
                ):
        ...
        try:
            ...
            with ArchiverRedisLock(
                ...
                if not relays_config:
                    logger.warning(
                        "No relays found in Redis, checking if we need to initialize from config"
                    )
                    ...
                    if config_relays:
                        try:
                            ...
                            logger.info(
                                f"Found {len(relays_dict)} relays in config:relays, copying to dvmdash:settings:relays"
                            )
                            ...
                            logger.error(
                                f"Error decoding config:relays: {config_relays}"
                            )
                ...
                if not relays_config:
                    ...
                    self.redis.set(
                        "dvmdash:settings:relays", json.dumps(default_relays)
                    )
                    relays_config = default_relays
                    logger.info(
                        f"Created default relay configuration with {len(default_relays)} relays"
                    )
                ...
                logger.info(
                    f"Found {len(high_activity)} high-activity relays and {len(normal_activity)} normal relays"
                )

                # Calculate distribution
                assignments = {c: [] for c in collectors}
                max_relays_per_collector = int(
                    os.getenv("MAX_RELAYS_PER_COLLECTOR", "5")
                )  # Maximum number of relays per collector

                # First distribute high-activity relays
                collector_index = 0
                for relay in high_activity:
                    while (
                        len(assignments[collectors[collector_index]])
                        >= max_relays_per_collector
                    ):
                ...
                for relay in normal_activity:
                    ...
                    while (
                        ...
                        if all(
                            len(assignments[c]) >= max_relays_per_collector
                            for c in collectors
                        ):
                            logger.info(
                ...
                with self.redis.pipeline() as pipe:
                    for collector_id, relays in assignments.items():
                        ...
                        pipe.set(
                            f"dvmdash:collector:{collector_id_str}:relays", relay_json
                        )

                        # Update collector's config version in the hash
                        current_version = self.redis.get(
                            "dvmdash:settings:config_version"
                        )
                        if current_version:
                            pipe.hset(
                                f"dvmdash:collector:{collector_id_str}",
                        ...
                        logger.info(
                            f"Assigned {len(relays)} relays to collector {collector_id}"
                        )
            ...
            for collector_id in collectors:
                ...
                if has_heartbeat:
                    if heartbeat_value < stale_threshold:
                        ...
                        logger.info(
                            f"Collector {collector_id_str} has stale heartbeat: {heartbeat_value} < {stale_threshold}"
                        )
                    ...
                    if has_registration:
                        ...
                        if current_time - registration_time > self.collector_timeout:
                            logger.info(
                                f"Collector {collector_id_str} has no heartbeat and was registered {current_time - registration_time}s ago, marking as stale"
                            )
                            stale_collectors.append(collector_id)
                        else:
                            logger.info(
                                f"Collector {collector_id_str} has no heartbeat but was recently registered ({current_time - registration_time}s ago), not marking as stale yet"
                            )
                    else:
                        # If we can't determine when it was added, be conservative and mark as stale
                        logger.info(
                            f"Collector {collector_id_str} has no heartbeat and no registration time, marking as stale"
                        )
    ...
    async def _cleanup_collector_metrics(self, stale_collectors: List) -> None:
        ...
        try:
            for collector_id in stale_collectors:
                ...
                if metrics_keys:
                    logger.info(
                        f"Cleaning up {len(metrics_keys)} metrics keys for stale collector {collector_id_str}"
                    )
                    ...
                    for i in range(0, len(metrics_keys), batch_size):
                        ...
                        if batch:
                            ...
                            logger.debug(
                                f"Deleted batch of {len(batch)} metrics keys for collector {collector_id_str}"
                            )
                ...
                if alt_metrics_keys:
                    logger.info(
                        f"Cleaning up {len(alt_metrics_keys)} alternate metrics keys for stale collector {collector_id_str}"
                    )
                    ...
                    for i in range(0, len(alt_metrics_keys), batch_size):
                        ...
                        if batch:
                            self.redis.delete(*batch)
                            logger.debug(
                                f"Deleted batch of {len(batch)} alternate metrics keys for collector {collector_id_str}"
                            )
...
class EventCollectorCoordinatorManager:
    ...
    def _initialize_nsec_keys(self):
        ...
        if not nsec_keys:
            logger.warning(
                "No DVMDASH_LISTENER_NSECS environment variable set, collectors will generate their own keys"
            )
            return

        keys_list = nsec_keys.split(",")
        logger.info(
            f"Initializing {len(keys_list)} nsec keys from environment variable"
        )
    ...
    def assign_nsec_key(self, collector_id):
        ...
        for key_id, key_value in all_keys.items():
            key_str = (
                key_value.decode("utf-8") if isinstance(key_value, bytes) else key_value
            )
    ...
    async def process_forever(self):
        ...
        while True:
            try:
                ...
                if current_time - self.last_relay_check >= self.relay_check_interval:
                    if await self.relay_coordinator.should_redistribute_relays():
                        ...
                        if distribution_success:
                            collectors = (
                                await self.relay_coordinator.get_active_collectors()
                            )
                            for collector_id in collectors:
                                collector_id_str = (
                                    collector_id.decode("utf-8")
                ...
                if (
                    current_time - self.last_global_cleanup
                    >= self.global_cleanup_interval
                ):
                ...
                if current_time - last_health_log >= 60:
                    ...
                    try:
                        ...
                        logger.info(
                            f"Current relay assignments for {len(collectors)} active collectors:"
                        )
                        ...
                        for collector_id in collectors:
                            ...
                            relays_json = self.redis.get(
                                f"dvmdash:collector:{collector_id_str}:relays"
                            )
                            relays = json.loads(relays_json) if relays_json else {}

                            logger.info(
                                f"  Collector {collector_id_str}: {len(relays)} relays assigned, last heartbeat: {heartbeat_age}"
                            )
    ...
    async def _perform_global_cleanup(self):
        ...
        try:
            ...
            logger.info(
                f"Found {len(all_metrics_keys)} total metrics keys during global cleanup"
            )
            ...
            if orphaned_keys:
                logger.info(
                    f"Found {len(orphaned_keys)} orphaned metrics keys to clean up"
                )
                ...
                for i in range(0, len(orphaned_keys), batch_size):
                    ...
                    if batch:
                        self.redis.delete(*batch)
                        logger.info(
                            f"Deleted batch of {len(batch)} orphaned metrics keys"
                        )
            ...
            if alt_metrics_keys:
                ...
                if orphaned_alt_keys:
                    logger.info(
                        f"Found {len(orphaned_alt_keys)} orphaned alternate metrics keys to clean up"
                    )
                    ...
                    for i in range(0, len(orphaned_alt_keys), batch_size):
                        ...
                        if batch:
                            self.redis.delete(*batch)
                            logger.info(
                                f"Deleted batch of {len(batch)} orphaned alternate metrics keys"
                            )
                    ...
                    logger.info(
                        "No orphaned alternate metrics keys found during global cleanup"
                    )
